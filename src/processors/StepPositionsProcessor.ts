import { StepPositionsRequest, StepPositionsResponse } from "../gen/messages_pb";
...
export class StepPositionsProcessor {
    ...
    private createStepPostionsResponse(positions: { stepValue: string; span: Range; }[]): StepPositionsResponse {
        const res = new StepPositionsResponse();

        res.setError("");
        res.setSteppositionsList(positions.map((p) => {
