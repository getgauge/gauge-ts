...
export abstract class HookExecutionProcessor extends ExecutionProcessor {
    ...
    public async process(req: HookExectionRequest): Promise<ExecutionStatusResponse> {
        const res = await this.executeHooks(req);

        return this.createExecutionResponse(res);
    }

    private async executeHooks(req: HookExectionRequest): Promise<ProtoExecutionResult> {
        ...
        try {
            for (const hook of hooks) {
                await this.executeMethod(hook.getInstance() as Record<string, unknown>,
    ...
    private getExecutionContext(info: ExecutionInfo): ExecutionContext {
        ...
        return new ExecutionContext(this.toSpec(specInfo),
            this.toScenario(scenarioInfo),
            this.toStepInfo(stepInfo), trace
        );
    }
    ...
    private toSpec(specInfo: SpecInfo | undefined): Specification | null {
        ...
        return new Specification(info.getName(), info.getFilename(),
            info.getIsfailed(), info.getTagsList()
        );
    }
    ...
    private toStepInfo(stepInfo: ProtoStepInfo | undefined): StepInfo | null {
        ...
        if (info.getStep()) {
            ...
            return new StepInfo(step.getParsedsteptext(),
                step.getActualsteptext(), stepInfo.getIsfailed(),
                stepInfo.getErrormessage(), stepInfo.getStacktrace()
            );
        }

        return new StepInfo(null, null, info.getIsfailed(),
            stepInfo.getErrormessage(), stepInfo.getStacktrace()
        );
    }
